...
class QuantizedDCNN(nn.Module):
    ...
    def _set_structure(self):
        self.conv1 = QuantizedConv2d(self.input_channels, self.structure[0], kernel_size=3, padding=1)
        self.conv2 = QuantizedConv2d(self.structure[0], self.structure[1], kernel_size=3, padding=1)
        self.maxpool0 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.bn0 = nn.BatchNorm2d(self.structure[1])

        self.conv3 = QuantizedConv2d(self.structure[1], self.structure[2], kernel_size=3, padding=1)
        self.conv4 = QuantizedConv2d(self.structure[2], self.structure[3], kernel_size=3, padding=1)
        self.maxpool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.bn1 = nn.BatchNorm2d(self.structure[3])

        self.conv5 = QuantizedConv2d(self.structure[3], self.structure[4], kernel_size=3, padding=1)
